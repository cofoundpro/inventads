        ...
        $('.wpaicg_check_openrouter_limits').click(function() {
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    if (response.success) {
                        var limitsHtml = 
                            '<strong>API Key:</strong> ' + response.data.label + '<br>' +
                            '<strong>Usage:</strong> ' + response.data.usage + ' credits used<br>' +
                            '<strong>Limit:</strong> ' + (response.data.limit !== null ? response.data.limit + ' credits' : 'Unlimited') + '<br>' +
                            '<strong>Requests allowed:</strong> ' + response.data.rate_limit.requests + '<br>' +
                            '<strong>Interval:</strong> ' + response.data.rate_limit.interval;
                        
                        $('#limitsDisplay').html(limitsHtml).show(); // Display the limits
                    } else {
                        $('#limitsDisplay').html('<strong>Error:</strong> ' + (response.data || 'An error occurred.')).show();
                    }
                },
                error: function(jqXHR, textStatus, errorThrown) {
                    btn.html(originalContent); // Restore original button text
                    $('#limitsDisplay').html('<strong>Error:</strong> ' + errorThrown).show();
                }
            });
        });
                    ...
                    if(response.next_step === 'DONE'){
                        ...
                        if(data.step === 'image'){
                            if(wpaicg_image_source === 'dalle' || wpaicg_image_source === 'dalle2' || wpaicg_image_source === 'dalle3' || wpaicg_image_source === 'dalle3hd'){
                                wpaicg_generatorProcess('image','<?php echo esc_html__('Generating Image with DALL-E','gpt3-ai-content-generator')?>');
                            }
                            ...
                            if(wpaicg_image_source === 'replicate'){
                                wpaicg_generatorProcess('image','<?php echo esc_html__('Generating Image with Replicate','gpt3-ai-content-generator')?>');
                            }
                        }
                        if(data.step === 'featuredimage'){
                            ...
                            if(wpaicg_featured_image_source === 'replicate'){
                                wpaicg_generatorProcess('featuredimage','<?php echo esc_html__('Generating Featured Image with Replicate','gpt3-ai-content-generator')?>');
                            }
                        }
                        oldStep = data.step;
