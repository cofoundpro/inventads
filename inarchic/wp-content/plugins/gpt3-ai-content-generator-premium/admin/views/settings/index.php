...
if ( isset( $_POST['wpaicg_submit'] ) ) {
    ...
    $option_mappings = [
        ...
        'wpaicg_default_replicate_model' => 'wpaicg_default_replicate_model',
    ...
    $current_model = $current_provider === 'OpenAI' ?
                    get_option('wpaicg_ai_model', 'gpt-3.5-turbo') :
                    ($current_provider === 'Google' ?
                    get_option('wpaicg_google_default_model', 'gemini-pro') : 
                    ''); // Default model for Google or other providers

    $settings_to_update = [
        'wpaicg_temperature' => ['key' => 'temperature', 'filter' => 'floatval'],
        'wpaicg_max_tokens' => ['key' => 'max_tokens', 'filter' => 'floatval'],
        'wpaicg_top_p' => ['key' => 'top_p', 'filter' => 'floatval'],
        'wpaicg_frequency' => ['key' => 'frequency_penalty', 'filter' => 'floatval'],
        'wpaicg_presence' => ['key' => 'presence_penalty', 'filter' => 'floatval'],
        'wpaicg_best_of' => ['key' => 'best_of', 'filter' => 'intval'],
        'wpai_language' => ['key' => 'wpai_language', 'filter' => 'sanitize_text_field'],
        'wpai_content_style' => ['key' => 'wpai_writing_style', 'filter' => 'sanitize_text_field'],
        'wpai_content_tone' => ['key' => 'wpai_writing_tone', 'filter' => 'sanitize_text_field'],
        'wpai_number_of_heading' => ['key' => 'wpai_number_of_heading', 'filter' => 'intval'],
        'wpai_heading_tag' => ['key' => 'wpai_heading_tag', 'filter' => 'sanitize_text_field'],
        'wpai_cta_pos' => ['key' => 'wpai_cta_pos', 'filter' => 'sanitize_text_field'],
        'wpaicg_img_size' => ['key' => 'img_size', 'filter' => 'sanitize_text_field'],
    ];
    ...
    foreach ($checkboxFields as $field) {
        ...
        $wpdb->update($table_name, [$field => $value], ['name' => 'wpaicg_settings']);
    }
...
if ( isset( $_POST['wpaicg_reset'] ) ) {
    ...
    $wpdb->insert($table_name, array(
        'name' => 'wpaicg_settings',
        'temperature' => 1, // Default value for temperature
        'max_tokens' => 1500,
        'top_p' => 0.01,
        'best_of' => 1,
        'frequency_penalty' => 0,
    ...
    update_option('_wpaicg_genesis_description', false);
    ...
    update_option('wpaicg_default_replicate_model', '');
...
$wpaicg_google_model_list = get_option('wpaicg_google_model_list', ['gemini-pro']); // Get Google model list
$wpaicg_google_default_model = get_option('wpaicg_google_default_model', 'gemini-pro');

$wpaicg_openrouter_api_key = get_option('wpaicg_openrouter_api_key', ''); // Get OpenRouter API Key
$wpaicg_openrouter_model_list = get_option('wpaicg_openrouter_model_list', []); // Get OpenRouter model list
...
$_genesis_description = get_option('_wpaicg_genesis_description',false);


if(!get_option('_wpaicg_shorten_url',false)){
    add_option('_wpaicg_shorten_url',false,'','no');
}
$_wpaicg_shorten_url = get_option('_wpaicg_shorten_url',false);

if(!get_option('_wpaicg_gen_title_from_keywords',false)){
    add_option('_wpaicg_gen_title_from_keywords',false,'','no');
}
...
if(!get_option('_wpaicg_original_title_in_prompt',false)){
    add_option('_wpaicg_original_title_in_prompt',false,'','no');
}
...
if(!get_option('wpaicg_enforce_woo_keyword_in_url',false)){
    add_option('wpaicg_enforce_woo_keyword_in_url',false,'','no');
}
...
if(!get_option('wpaicg_woo_custom_prompt',false)){
    add_option('wpaicg_woo_custom_prompt',false,'','no');
}
...
if(!get_option('wpaicg_order_status_token',false)){
    add_option('wpaicg_order_status_token',false,'','no');
}
...
if(!get_option('wpaicg_sd_api_version',false)){
    add_option('wpaicg_sd_api_version',false,'','no');
}
$wpaicg_sd_api_version = get_option('wpaicg_sd_api_version','');

if(!get_option('wpaicg_default_replicate_model',false)){
    add_option('wpaicg_default_replicate_model',false,'','no');
}
...
if(!get_option('wpaicg_comment_prompt',false)){
    add_option('wpaicg_comment_prompt',false,'','no');
}
...
if(!get_option('wpaicg_search_placeholder',false)){
    add_option('wpaicg_search_placeholder',false,'','no');
}
...
if(!get_option('wpaicg_search_result_font_color',false)){
    add_option('wpaicg_search_result_font_color',false,'','no');
}
...
if(!get_option('wpaicg_search_result_bg_color',false)){
    add_option('wpaicg_search_result_bg_color',false,'','no');
}
...
if(!get_option('wpaicg_search_loading_color',false)){
    add_option('wpaicg_search_loading_color',false,'','no');
}
...
<div class="demo-page-master">
      ...
      <ul>
        <li>
          <a href="#settings">
            ...
            <?php echo esc_html__('AI Engine','gpt3-ai-content-generator')?>
            </a>
        </li>
        <li>
          <a href="#content">
            ...
            <?php echo esc_html__('Content Writer','gpt3-ai-content-generator')?>
        </a>
        </li>
        <li>
          <a href="#seo">
            ...
            <?php echo esc_html__('SEO','gpt3-ai-content-generator')?>
        </a>
        </li>
        <li>
          <a href="#image">
            ...
            <?php echo esc_html__('Image','gpt3-ai-content-generator')?>
        </a>
        </li>
        <li>
          <a href="#woocommerce">
            ...
            <?php echo esc_html__('WooCommerce','gpt3-ai-content-generator')?>
            </a>
        </li>
        <li>
          <a href="#assistant">
            ...
            <?php echo esc_html__('AI Assistant','gpt3-ai-content-generator')?>
        </a>
        </li>
        <li>
          <a href="#tools">
          <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-layout">
            ...
            <?php echo esc_html__('Tools','gpt3-ai-content-generator')?>
        </a>
        </li>
      </ul>
    </nav>
  ...
  <main class="demo-page-master-content">
    <form action="" method="post">
        <?php wp_nonce_field('wpaicg_setting_save'); ?>
        ...
        <section>
            ...
            <div class="unique-page-container">
                <div class="nice-form-group">
                    <label for="wpaicg_provider"><?php echo esc_html__('Provider', 'gpt3-ai-content-generator'); ?></label>
                    <select id="wpaicg_provider" name="wpaicg_provider" class="specific-select">
                        ...
                        <option value="Azure" <?php selected($wpaicg_provider, 'Azure'); ?>>Microsoft</option>
                    </select>
            ...
            <div id="openai_specific_fields">
                ...
                <div class="unique-page-container">
                    <div class="nice-form-group">
                        <label><?php echo esc_html__('Api Key','gpt3-ai-content-generator')?></label>
                        <input id="wpaicg_openai_api_key" name="wpaicg_openai_api_key" type="text" value="<?php echo esc_attr($current_openai_api_key); ?>" onfocus="unmaskValue(this)" onblur="maskValue(this)" class="specific-textfield">
                        <a href="https://beta.openai.com/account/api-keys" target="_blank"><?php echo esc_html__('Get Your Api Key','gpt3-ai-content-generator')?></a>
                    </div>
                </div>
                <!-- OPENAI MODELS -->
                <div class="unique-page-container">
                    <div class="nice-form-group">
                        <label for="wpaicg_ai_model"><?php echo esc_html__('Model','gpt3-ai-content-generator')?></label>
            ...
            <div id="google_specific_fields" style="display: none;">
                ...
                <div class="unique-page-container">
                    <div class="nice-form-group">
                        <label><?php echo esc_html__('Api Key','gpt3-ai-content-generator')?></label>
                        <input id="wpaicg_google_api_key" name="wpaicg_google_api_key" type="text" value="<?php echo esc_attr($wpaicg_google_api_key); ?>" onfocus="unmaskValue(this)" onblur="maskValue(this)" class="specific-textfield">
                        <a href="https://aistudio.google.com/app/apikey" target="_blank"><?php echo esc_html__('Get Your Api Key','gpt3-ai-content-generator')?></a>
                    </div>
                </div>
                <!-- GOOGLE MODELS -->
                <div class="unique-page-container">
                    <div class="nice-form-group">
                        ...
                        <?php if (isset($wpaicg_google_model_list) && is_array($wpaicg_google_model_list) && count($wpaicg_google_model_list) > 0):
                            foreach ($wpaicg_google_model_list as $model):
                                // Define words that trigger disabling the option
                                $disabledWords = ['vision']; // Add words that should disable the option
                                $shouldBeDisabled = false;
                                foreach ($disabledWords as $word) {
                                    if (strpos($model, $word) !== false) {
                                        $shouldBeDisabled = true;
                                        break; // Break the loop if any word is found
                                    }
                                }
                            ?>
                                <option value="<?php echo esc_attr($model); ?>" <?php selected($model, $wpaicg_google_default_model); ?> <?php echo $shouldBeDisabled ? 'disabled' : ''; ?>>
                                    <?php echo esc_html(ucwords(str_replace('-', ' ', $model))); // Convert save format to display format ?>
                                </option>
                            <?php endforeach; else: ?>
                        ...
                        <input type="hidden" name="wpaicg_google_model_list" value="<?php echo isset($wpaicg_google_model_list) && is_array($wpaicg_google_model_list) ? esc_attr(implode(',', $wpaicg_google_model_list)) : ''; ?>">
                    </div>
                </div>
            </div>
            ...
            <div id="azure_specific_fields" style="display: none;">
                ...
                <div class="unique-page-container">
                    <div class="nice-form-group">
                        <label><?php echo esc_html__('Api Key','gpt3-ai-content-generator')?></label>
                        <input type="text" id="wpaicg_azure_api_key" class="specific-textfield" name="wpaicg_azure_api_key" value="<?php echo esc_attr($wpaicg_azure_api_key); ?>" onfocus="unmaskValue(this)" onblur="maskValue(this)" >
                        <a href="https://azure.microsoft.com/en-us/products/ai-services/openai-service" target="_blank"><?php echo esc_html__('Get Your Api Key','gpt3-ai-content-generator')?></a>
                    </div>
                </div>
                <!-- AZURE ENDPOINT -->
                <div class="unique-page-container">
                    <div class="nice-form-group">
                        <label><?php echo esc_html__('Endpoint', 'gpt3-ai-content-generator'); ?></label>
                        <input type="text" id="wpaicg_azure_endpoint" name="wpaicg_azure_endpoint" value="<?php echo esc_attr($wpaicg_azure_endpoint); ?>" class="specific-textfield">
                ...
                <div class="unique-page-container">
                    <div class="nice-form-group">
                        ...
                        <input type="text" id="wpaicg_azure_deployment" name="wpaicg_azure_deployment" value="<?php echo esc_attr($wpaicg_azure_deployment); ?>" class="specific-textfield">
                        <a href="https://docs.aipower.org/docs/ai-engine/azure-openai" target="_blank">?</a>
                    </div>
                </div>
                <!-- AZURE EMBEDDINGS -->
                <div class="unique-page-container">
                    <div class="nice-form-group">
                        <label><?php echo esc_html__('  Embeddings', 'gpt3-ai-content-generator'); ?></label>
                        <input type="text" id="wpaicg_azure_embeddings" name="wpaicg_azure_embeddings" value="<?php echo esc_attr($wpaicg_azure_embeddings); ?>" class="specific-textfield">
                        <a href="https://docs.aipower.org/docs/ai-engine/azure-openai" target="_blank">?</a>
                    </div>
                </div>
            </div>
            ...
            <div id="openrouter_specific_fields" style="display: none;">
                ...
                <div class="unique-page-container">
                    <div class="nice-form-group">
                        <label><?php echo esc_html__('Api Key','gpt3-ai-content-generator')?></label>
                        <input type="text" id="wpaicg_openrouter_api_key" class="specific-textfield" name="wpaicg_openrouter_api_key" value="<?php echo esc_attr($wpaicg_openrouter_api_key); ?>" onfocus="unmaskValue(this)" onblur="maskValue(this)" >
                        <a href="https://openrouter.ai/keys" target="_blank"><?php echo esc_html__('Get Your Api Key','gpt3-ai-content-generator')?></a>
                    </div>
                </div>
                    ...
                    <div class="nice-form-group">
                        ...
                        <select id="wpaicg_openrouter_model" name="wpaicg_openrouter_model" class="specific-select">
                            ...
                            foreach ($openrouter_models as $model) {
                                $provider = explode('/', $model['id'])[0]; // Extract the provider name
                        ...
                        <button id="checkLimitsButton" class="button button-secondary wpaicg_check_openrouter_limits" type="button" style="height: 40px; margin-left: 2px;">
                            Limits
                        </button>
                        <!-- Container for displaying limits -->
                        <div id="limitsDisplay" style="margin-top: 15px; padding: 10px; border: 1px solid #ccc; display: none;"></div>
                    </div>
                </div>
            </div>
            <p></p>
            <div class="advance-button-container">
                ...
                <div class="advanced-settings" id="toggleSingleSettings" data-target="single-settings-container">
                    <?php echo esc_html__('Advance Settings','gpt3-ai-content-generator')?>
                </div>
                ...
                <div class="safety-settings" id="toggleGoogleSafetySettings" data-target="google_safety_settings" style="display: none;">
                    <?php echo esc_html__('Safety Settings','gpt3-ai-content-generator')?>
                </div>
            </div>
            <div class="single-settings-container" style="display: none;">
                ...
                <div class="unique-page-container">
                    <div class="nice-form-group">
                        <label><?php echo esc_html__('Maximum Length','gpt3-ai-content-generator')?></label>
                        <input id="wpaicg_max_tokens" name="wpaicg_max_tokens" type="number" class="specific-textfield" value="<?php echo esc_attr($current_max_tokens); ?>">
                        <a href="https://docs.aipower.org/docs/ai-engine/openai/max-tokens#adjusting-the-max-tokens-setting" target="_blank">?</a>
                    </div>
                </div>

                <!-- RATE LIMIT -->
                <?php $wpaicg_sleep_time = get_option('wpaicg_sleep_time',1); ?>
                <div class="nice-form-group">
                    <label><?php echo esc_html__('Rate Limit Buffer (in seconds)','gpt3-ai-content-generator')?></label>
                    <input id="wpaicg_sleep_time_range" name="wpaicg_sleep_time" type="range" min="1" max="30" value="<?php echo esc_attr($wpaicg_sleep_time); ?>" oninput="this.nextElementSibling.value = this.value">
                ...
                <div class="nice-form-group">
                    ...
                    <input id="wpaicg_temperature_range" name="wpaicg_temperature" type="range" step="0.01" min="0" max="2" value="<?php echo esc_attr($current_temperature); ?>" oninput="this.nextElementSibling.value = this.value">
                ...
                <div class="nice-form-group">
                    ...
                    <input id="wpaicg_frequency_range" name="wpaicg_frequency" type="range" step="0.01" min="0" max="2" value="<?php echo esc_attr($current_frequency); ?>" oninput="this.nextElementSibling.value = this.value">
                ...
                <div class="nice-form-group">
                    ...
                    <input id="wpaicg_presence_range" name="wpaicg_presence" type="range" step="0.01" min="0" max="2" value="<?php echo esc_attr($current_presence); ?>" oninput="this.nextElementSibling.value = this.value">
                ...
                <div class="nice-form-group">
                    ...
                    <input id="wpaicg_top_p_range" name="wpaicg_top_p" type="range" step="0.01" min="0" max="1" value="<?php echo esc_attr($current_top_p); ?>" oninput="this.nextElementSibling.value = this.value">
                    <output><?php echo esc_attr($current_top_p); ?></output>
                </div>

                <!-- BEST_OF -->
                <div class="nice-form-group">
                    <label><?php echo esc_html__('Best of','gpt3-ai-content-generator')?></label>
                    <input id="wpaicg_best_of" name="wpaicg_best_of" type="range" min="1" max="20" value="<?php echo esc_attr($current_best_of); ?>" oninput="this.nextElementSibling.value = this.value">
                    <output><?php echo esc_attr($current_best_of); ?></output>
                </div>
            </div>
            ...
            <div id="google_safety_settings" style="display: none;">
                ...
                <div class="unique-page-container">
                    <div class="nice-form-group">
                        ...
                        <select id="<?php echo esc_attr($category); ?>" name="google_safety_settings[<?php echo esc_attr($category); ?>]" class="specific-select">
            ...
            <details> 
                <summary>
                    <input type="submit" value="<?php echo esc_html__('Save','gpt3-ai-content-generator')?>" name="wpaicg_submit" class="button button-primary button-large">
        ...
        <section>
            ...
            <p><?php echo esc_html__('This tab allows you to set and save default values for both Express Mode and Auto Content Writer. Changes made here will be applied to both modules.','gpt3-ai-content-generator')?></p>
            <h1><?php echo esc_html__('Language, Style and Tone','gpt3-ai-content-generator')?></h1>
            <div class="nice-form-group" style="display: flex;justify-content: space-between;margin-top: -1em;">
                ...
                <div class="unique-page-container">
                    <div class="nice-form-group">
                        ...
                        <select class="specific-select" id="wpai_language" name="wpai_language" style="width: 162px;">
                            <?php foreach ($languages as $code => $displayName): ?>
                                <option value="<?php echo esc_attr($code); ?>" <?php echo esc_attr($code) === $currentLanguage ? 'selected' : ''; ?>><?php echo esc_html($displayName); ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                </div>
                ...
                <div class="unique-page-container">
                    <div class="nice-form-group">
                        <label><?php echo esc_html__('Writing Style','gpt3-ai-content-generator')?></label>
                        <select class="specific-select" id="wpai_content_style" name="wpai_content_style" style="width: 162px;">
                            <?php foreach ($writing_styles as $code => $displayName): ?>
                                <option value="<?php echo esc_attr($code); ?>" <?php echo esc_attr($code) === $currentStyle ? 'selected' : ''; ?>><?php echo esc_html($displayName); ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                </div>
                ...
                <div class="unique-page-container">
                    <div class="nice-form-group">
                        <label><?php echo esc_html__('Writing Tone','gpt3-ai-content-generator')?></label>
                        <select class="specific-select" id="wpai_content_tone" name="wpai_content_tone" style="width: 162px;">
                            <?php foreach ($writing_tones as $code => $displayName): ?>
                                <option value="<?php echo esc_attr($code); ?>" <?php echo esc_attr($code) === $currentTone ? 'selected' : ''; ?>><?php echo esc_html($displayName); ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                </div>
            ...
            <p></p>
            <h1><?php echo esc_html__('Headings','gpt3-ai-content-generator')?></h1>
            <!-- HEADINGS -->
            <div class="unique-page-container">
                <div class="nice-form-group">
                    <label><?php echo esc_html__('Number of Headings', 'gpt3-ai-content-generator'); ?></label>
                    <input id="wpai_number_of_heading" name="wpai_number_of_heading" type="range" min="1" max="15" value="<?php echo esc_attr($current_number_of_heading); ?>" oninput="this.nextElementSibling.value = this.value">
                    <output><?php echo esc_attr($current_number_of_heading); ?></output>
                </div>
            </div>
            <!-- HEADING TAG -->
            <div class="unique-page-container">
                <div class="nice-form-group">
                    <label><?php echo esc_html__('Heading Tag','gpt3-ai-content-generator')?></label>
                    <select class="specific-select" id="wpai_heading_tag" name="wpai_heading_tag">
                        <?php foreach ($heading_tags as $tag): ?>
                            <option value="<?php echo esc_attr($tag); ?>" <?php echo $tag === $currentTag ? 'selected' : ''; ?>>
                                <?php echo $tag; ?>
                            </option>
                        <?php endforeach; ?>
                    </select>
            ...
            <fieldset class="nice-form-group">
                ...
                <!-- TABLE OF CONTENTS -->
                <div class="nice-form-group">
                    <input type="checkbox" id="wpaicg_toc" name="wpaicg_toc" value="1" <?php checked(1, $current_wpaicg_toc); ?> />
                    <label><?php echo esc_html__('Table of Contents','gpt3-ai-content-generator')?></label>
                ...
                <div class="unique-page-container" id="toc_title_container">
                    <div class="nice-form-group">
                        <label><?php echo esc_html__('ToC Title', 'gpt3-ai-content-generator'); ?></label>
                        <input type="text" id="wpaicg_toc_title" name="wpaicg_toc_title" value="<?php echo esc_attr($current_toc_title); ?>" class="specific-textfield">
                    </div>
                </div>
                <!-- ToC Title TAG -->
                <div class="unique-page-container" id="toc_title_tag_container">
                    <div class="nice-form-group">
                        <label><?php echo esc_html__('ToC Tag','gpt3-ai-content-generator')?></label>
                ...
                <!-- Add Intro -->
                <div class="nice-form-group">
                    <input type="checkbox" id="wpai_add_intro" name="wpai_add_intro" value="1" <?php checked(1, $current_wpaicg_intro); ?> />
                    <label><?php echo esc_html__('Introduction','gpt3-ai-content-generator')?></label>
                ...
                <div class="unique-page-container" id="hide_intro_title_container">
                    <div class="nice-form-group">
                        <input type="checkbox" id="wpaicg_hide_introduction" name="wpaicg_hide_introduction" value="1" <?php checked(1, $current_hide_introduction); ?> />
                        <label><?php echo esc_html__('Hide Introduction Title', 'gpt3-ai-content-generator'); ?></label>
                    </div>
                </div>
                <div class="unique-page-container" id="intro_title_tag_container">
                    <div class="nice-form-group">
                        <label><?php echo esc_html__('Introduction Tag','gpt3-ai-content-generator')?></label>
                ...
                <!-- Add Conclusion -->
                <div class="nice-form-group">
                    <input type="checkbox" id="wpai_add_conclusion" name="wpai_add_conclusion" value="1" <?php checked(1, $current_wpaicg_conclusion); ?> />
                    <label><?php echo esc_html__('Conclusion','gpt3-ai-content-generator')?></label>
                ...
                <div class="unique-page-container" id="hide_conclusion_title_container">
                    <div class="nice-form-group">
                        ...
                        <label><?php echo esc_html__('Hide Conclusion Title', 'gpt3-ai-content-generator'); ?></label>
                    </div>
                </div>
                <div class="unique-page-container" id="conclusion_title_tag_container">
                    <div class="nice-form-group">
                        <label><?php echo esc_html__('Conclusion Tag','gpt3-ai-content-generator')?></label>
                        <select class="specific-select" id="wpaicg_conclusion_title_tag" name="wpaicg_conclusion_title_tag">
                            <?php foreach ($heading_tags as $tag): ?>
                                <option value="<?php echo esc_attr($tag); ?>" <?php echo $tag === $current_conclusion_title_tag ? 'selected' : ''; ?>>
                                    <?php echo $tag; ?>
                                </option>
                            <?php endforeach; ?>
                        </select>
                ...
                <div class="nice-form-group">
                    <input type="checkbox" id="wpai_add_tagline" name="wpai_add_tagline" value="1" <?php checked(1, $current_tagline); ?> />
                    <label><?php echo esc_html__('Tagline','gpt3-ai-content-generator')?></label>
                ...
                <div class="nice-form-group">
                    <input type="checkbox" id="wpai_modify_headings" name="wpai_modify_headings" value="1" <?php checked(1, $current_outline_editor); ?> />
                    <label><?php echo esc_html__('Outline Editor','gpt3-ai-content-generator')?></label>
                    <a href="https://docs.aipower.org/docs/content-writer/express-mode/headings#outline-editor" target="_blank">?</a>
                </div>
                <div class="nice-form-group">
                <?php if (wpaicg_gacg_fs()->is_plan__premium_only('pro')) { ?>
                    <input type="checkbox" id="wpai_add_faq" name="wpai_add_faq" value="1" <?php checked(1, $current_qa); ?> />
                    <label><?php echo esc_html__('Q & A','gpt3-ai-content-generator')?></label>
                ...
                <div class="nice-form-group">
                <?php if (wpaicg_gacg_fs()->is_plan__premium_only('pro')) { ?>
                    <input type="checkbox" id="wpai_add_keywords_bold" name="wpai_add_keywords_bold" value="1" <?php checked(1, $current_bold_keywords); ?> />
            ...
            <!-- CTA POSITION -->
            <fieldset class="nice-form-group">
                <legend><?php echo esc_html__('Call to Action Position','gpt3-ai-content-generator')?></legend>
                <div class="nice-form-group">
                    <input type="radio" id="cta_pos_beg" name="wpai_cta_pos" value="beg" <?php checked($current_cta_pos, 'beg'); ?>/>
                    <label><?php echo esc_html__('Beginning','gpt3-ai-content-generator')?></label>
                </div>
                <div class="nice-form-group">
                    <input type="radio" id="cta_pos_end" name="wpai_cta_pos" value="end" <?php checked($current_cta_pos, 'end'); ?>/>
                    <label><?php echo esc_html__('End','gpt3-ai-content-generator')?></label>
                </div>
            </fieldset>
            
            <!-- CUSTOM PROMPT -->
            <div class="nice-form-group">
                <input <?php echo $wpaicg_content_custom_prompt_enable ? ' checked':''?> type="checkbox" class="wpaicg_meta_custom_prompt_enable" name="wpaicg_content_custom_prompt_enable">
            ...
            <div class="wpaicg_meta_custom_prompt_box" style="<?php echo isset($wpaicg_content_custom_prompt_enable) && $wpaicg_content_custom_prompt_enable ? '':'display:none'?>">
                <textarea rows="20" class="wpaicg_meta_custom_prompt" name="wpaicg_content_custom_prompt"><?php echo esc_html(str_replace("\\",'',$wpaicg_content_custom_prompt))?></textarea>
                <?php
                if(\WPAICG\wpaicg_util_core()->wpaicg_is_pro()):
                    ?>
                    <div>
                        <?php echo sprintf(esc_html__('Make sure to include %s in your prompt. You can also incorporate %s and %s to further customize your prompt.','gpt3-ai-content-generator'),'<code>[title]</code>','<code>[keywords_to_include]</code>','<code>[keywords_to_avoid]</code>')?>
                    </div>
                <?php else: ?>
                    <div>
                        <?php echo sprintf(esc_html__('Ensure %s is included in your prompt.','gpt3-ai-content-generator'),'<code>[title]</code>')?>
                    </div>
                <?php endif; ?>
        ...
        <section>
            ...
            <!-- SEO -->
            <fieldset class="nice-form-group">
                <legend><?php echo esc_html__('Meta Description','gpt3-ai-content-generator')?></legend>
                <div class="nice-form-group">
                    <input type="checkbox" id="_wpaicg_seo_meta_desc" name="_wpaicg_seo_meta_desc" value="1" <?php checked(1, $_wpaicg_seo_meta_desc); ?> />
                    <label><?php echo esc_html__('Generate Meta Description','gpt3-ai-content-generator')?></label>
                ...
                <div class="nice-form-group">
                    ...
                    <label><?php echo esc_html__('Include Meta in the Header','gpt3-ai-content-generator')?></label>
                ...
                foreach ($seo_plugins_options as $seo_plugin) {
                    if (is_plugin_active($seo_plugin['plugin'])) {
                        ...
                        ?>
                        <div class="nice-form-group">
                            <input <?php checked($option_value, true); ?> id="<?php echo esc_attr($seo_plugin['option_name']); ?>" type="checkbox" name="<?php echo esc_attr($seo_plugin['option_name']); ?>" value="1">
                            <label><?php echo esc_html($seo_plugin['label']); ?></label>
            ...
            <fieldset class="nice-form-group">
                <legend><?php echo esc_html__('SEO Optimization','gpt3-ai-content-generator')?></legend>
                <div class="nice-form-group">
                ...
                <div class="nice-form-group">
                <?php if(\WPAICG\wpaicg_util_core()->wpaicg_is_pro()): ?>
                    <input type="checkbox" id="_wpaicg_gen_title_from_keywords" name="_wpaicg_gen_title_from_keywords" value="1" <?php checked(1, $_wpaicg_gen_title_from_keywords); ?> onchange="handleTitleFromKeywordsChange(this)" />
        ...
        <section>
            ...
            <fieldset class="nice-form-group">
                <legend><?php echo esc_html__('Product Writer','gpt3-ai-content-generator')?></legend>
                <div class="nice-form-group">
                    <input type="checkbox" id="wpaicg_woo_generate_title" name="wpaicg_woo_generate_title" value="1" <?php checked(1, $wpaicg_woo_generate_title); ?> />
                    <label><?php echo esc_html__('Product Title','gpt3-ai-content-generator')?></label>
                    <a href="https://docs.aipower.org/docs/woocommerce#woocommerce-product-writer" target="_blank">?</a>
                </div>
                <div class="nice-form-group">
                    <input type="checkbox" id="wpaicg_woo_generate_description" name="wpaicg_woo_generate_description" value="1" <?php checked(1, $wpaicg_woo_generate_description); ?> />
                    <label><?php echo esc_html__('Full Product Description','gpt3-ai-content-generator')?></label>
                    <a href="https://docs.aipower.org/docs/woocommerce#woocommerce-product-writer" target="_blank">?</a>
                </div>
                <div class="nice-form-group">
                    <input type="checkbox" id="wpaicg_woo_generate_short" name="wpaicg_woo_generate_short" value="1" <?php checked(1, $wpaicg_woo_generate_short); ?> />
                    <label><?php echo esc_html__('Short Product Description','gpt3-ai-content-generator')?></label>
                    <a href="https://docs.aipower.org/docs/woocommerce#woocommerce-product-writer" target="_blank">?</a>
                </div>
                <div class="nice-form-group">
                    <input type="checkbox" id="wpaicg_woo_generate_tags" name="wpaicg_woo_generate_tags" value="1" <?php checked(1, $wpaicg_woo_generate_tags); ?> />
                    <label><?php echo esc_html__('Product Tags','gpt3-ai-content-generator')?></label>
                    <a href="https://docs.aipower.org/docs/woocommerce#woocommerce-product-writer" target="_blank">?</a>
                </div>
            </fieldset>
            ...
            <fieldset class="nice-form-group">
                <legend><?php echo esc_html__('SEO Optimization','gpt3-ai-content-generator')?></legend>
                <div class="nice-form-group">
                    <input type="checkbox" id="wpaicg_woo_meta_description" name="wpaicg_woo_meta_description" value="1" <?php checked(1, $wpaicg_woo_meta_description); ?> />
                    <label><?php echo esc_html__('Meta Description','gpt3-ai-content-generator')?></label>
                    <a href="https://docs.aipower.org/docs/woocommerce#woocommerce-product-writer" target="_blank">?</a>
                </div>
                <div class="nice-form-group">
                <?php if(\WPAICG\wpaicg_util_core()->wpaicg_is_pro()): ?>
                    <input type="checkbox" id="_wpaicg_shorten_woo_url" name="_wpaicg_shorten_woo_url" value="1" <?php checked(1, $_wpaicg_shorten_woo_url); ?> />
                ...
                <div class="nice-form-group">
                <?php if(\WPAICG\wpaicg_util_core()->wpaicg_is_pro()): ?>
                    <input type="checkbox" id="wpaicg_generate_woo_focus_keyword" name="wpaicg_generate_woo_focus_keyword" value="1" <?php checked(1, $wpaicg_generate_woo_focus_keyword); ?> />
                ...
                <div class="nice-form-group">
                <?php if(\WPAICG\wpaicg_util_core()->wpaicg_is_pro()): ?>
                    <input type="checkbox" id="wpaicg_enforce_woo_keyword_in_url" name="wpaicg_enforce_woo_keyword_in_url" value="1" <?php checked(1, $wpaicg_enforce_woo_keyword_in_url); ?> />
                    <label><?php echo esc_html__('Enforce Focus Keyword in URL','gpt3-ai-content-generator')?></label>
            ...
            <fieldset class="nice-form-group">
            <legend><?php echo esc_html__('Prompt Design','gpt3-ai-content-generator')?></legend>
                <div class="nice-form-group">
                    <input type="checkbox" id="wpaicg_woo_custom_prompt" name="wpaicg_woo_custom_prompt" class="wpaicg_woo_custom_prompt" value="1" <?php checked(1, $wpaicg_woo_custom_prompt); ?> />
                    <label><?php echo esc_html__('Use Custom Prompt','gpt3-ai-content-generator')?></label>
            ...
            <div <?php echo $wpaicg_woo_custom_prompt ? '':' style="display:none"';?> class="wpaicg_woo_custom_prompts">
                <div class="nice-form-group">
                    <p><?php echo esc_html__('You can use these shortcodes in your custom prompts:', 'gpt3-ai-content-generator');?> </p>
                ...
                <div class="nice-form-group">
                    <label><?php echo esc_html__('Title Prompt Template','gpt3-ai-content-generator')?></label>
                ...
                <p></p>
                <div class="nice-form-group">
                    <label><?php echo esc_html__('Title Prompt','gpt3-ai-content-generator')?></label>
                    <textarea rows="5" type="text" name="wpaicg_woo_custom_prompt_title"><?php echo esc_html($wpaicg_woo_custom_prompt_title);?></textarea>
                </div>
                <!-- Added Short Description Prompt Templates Dropdown -->
                <div class="nice-form-group">
                    <label><?php echo esc_html__('Short Description Prompt Template','gpt3-ai-content-generator')?></label>
                ...
                <p></p>
                <div class="nice-form-group">
                    <label><?php echo esc_html__('Short Description Prompt','gpt3-ai-content-generator')?></label>
                    <textarea rows="10" type="text" name="wpaicg_woo_custom_prompt_short"><?php echo esc_html($wpaicg_woo_custom_prompt_short);?></textarea>
                </div>
                <p></p>
                <div class="nice-form-group">
                    <label><?php echo esc_html__('Description Prompt Template','gpt3-ai-content-generator')?></label>
                ...
                <div class="nice-form-group">
                    <label><?php echo esc_html__('Description Prompt','gpt3-ai-content-generator')?></label>
                    <textarea rows="10" type="text" name="wpaicg_woo_custom_prompt_description"><?php echo esc_html($wpaicg_woo_custom_prompt_description);?></textarea>
                </div>
                <p></p>
                <div class="nice-form-group">
                    <label><?php echo esc_html__('Meta Description Prompt Template','gpt3-ai-content-generator')?></label>
                ...
                <div class="nice-form-group">
                    <label><?php echo esc_html__('Meta Description Prompt','gpt3-ai-content-generator')?></label>
                    <textarea rows="5" type="text" name="wpaicg_woo_custom_prompt_meta"><?php echo esc_html($wpaicg_woo_custom_prompt_meta);?></textarea>
                </div>
                <p></p>
                <div class="nice-form-group">
                    <label><?php echo esc_html__('Tag Prompt Template','gpt3-ai-content-generator')?></label>
                ...
                <div class="nice-form-group">
                    <label><?php echo esc_html__('Tag Prompt','gpt3-ai-content-generator')?></label>
                    <textarea rows="5" type="text" name="wpaicg_woo_custom_prompt_keywords"><?php echo esc_html($wpaicg_woo_custom_prompt_keywords);?></textarea>
                </div>
                <p></p>
                <?php if (\WPAICG\wpaicg_util_core()->wpaicg_is_pro()): ?>
                <!-- Added Focus Keyword Prompt Templates Dropdown -->
                <div class="nice-form-group">
                    <label><?php echo esc_html__('Focus Keyword Prompt Template','gpt3-ai-content-generator')?></label>
                ...
                <div class="nice-form-group">
                    <label><?php echo esc_html__('Focus Keyword Prompt','gpt3-ai-content-generator')?></label>
                    <textarea rows="5" type="text" name="wpaicg_woo_custom_prompt_focus_keyword"><?php echo esc_html($wpaicg_woo_custom_prompt_focus_keyword);?></textarea>
                </div>
                <?php endif; ?>
            </div>
            
            <fieldset class="nice-form-group">
                <legend><?php echo esc_html__('Token Sale', 'gpt3-ai-content-generator'); ?></legend>
                <small><?php echo esc_html__("Automatically credit tokens to the user's account based on the order status.", 'gpt3-ai-content-generator'); ?></small>
                <div class="nice-form-group">
                    <input type="radio" id="order_status_completed" name="wpaicg_order_status_token" value="completed" <?php checked($wpaicg_order_status_token, 'completed'); ?>/>
                    <label for="order_status_completed"><?php echo esc_html__('Completed', 'gpt3-ai-content-generator'); ?></label>
                </div>
                <div class="nice-form-group">
                    <input type="radio" id="order_status_processing" name="wpaicg_order_status_token" value="processing" <?php checked($wpaicg_order_status_token, 'processing'); ?>/>
                    <label for="order_status_processing"><?php echo esc_html__('Processing', 'gpt3-ai-content-generator'); ?></label>
                </div>
            </fieldset>
        ...
        <section>
            ...
            <div class="unique-page-container">
                <div class="nice-form-group">
                    <label for="image_source"><?php echo esc_html__('Image Source','gpt3-ai-content-generator')?></label>
                    <select id="image_source" name="wpaicg_image_source" class="specific-select"><?php echo get_image_source_options( $wpaicg_image_source, 'dalle3' ); ?></select>
                    <a href="https://docs.aipower.org/docs/content-writer/express-mode/images#adding-an-image" target="_blank">?</a>
                </div>
            </div>
            <div class="unique-page-container">
                <div class="nice-form-group">
                    <label for="featured_image_source" ><?php echo esc_html__('Featured Image Source','gpt3-ai-content-generator')?></label>
                    <select id="featured_image_source" name="wpaicg_featured_image_source" class="specific-select"><?php echo get_image_source_options( $wpaicg_featured_image_source, 'dalle3' ); ?></select>
            ...
            <div class="dalle-settings-container" style="display: none;">
                ...
                <div class="unique-page-container">
                    <div class="nice-form-group">
                        <label for="wpaicg_img_size"><?php echo esc_html__('Size','gpt3-ai-content-generator')?></label>
                        <select class="specific-select" id="wpaicg_img_size" name="wpaicg_img_size">
                            <?php foreach ($image_sizes as $code => $displayName): ?>
                                <option value="<?php echo esc_attr($code); ?>" <?php echo esc_attr($code) === $current_img_size ? 'selected' : ''; ?>><?php echo esc_html($displayName); ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                </div>
                <div class="unique-page-container">
                    <div class="nice-form-group">
                        <label for="wpaicg_dalle_type"><?php echo esc_html__('Type','gpt3-ai-content-generator')?></label>
                ...
                <div class="unique-page-container">
                    <div class="nice-form-group">
                        <label for="_wpaicg_image_style"><?php echo esc_html__('Style','gpt3-ai-content-generator')?></label>
                ...
                <div class="unique-page-container">
                    <div class="nice-form-group">
                        <label for="artist"><?php echo esc_html__('Artist','gpt3-ai-content-generator')?></label>
                ...
                <div class="unique-page-container">
                    <div class="nice-form-group">
                        <label for="photography_style"><?php echo esc_html__('Photography','gpt3-ai-content-generator')?></label>
                ...
                <div class="unique-page-container">
                    <div class="nice-form-group">
                        <label for="lighting"><?php echo esc_html__('Lighting','gpt3-ai-content-generator')?></label>
                ...
                <div class="unique-page-container">
                    <div class="nice-form-group">
                        <label for="subject"><?php echo esc_html__('Subject','gpt3-ai-content-generator')?></label>
                ...
                <div class="unique-page-container">
                    <div class="nice-form-group">
                        <label for="camera_settings"><?php echo esc_html__('Camera','gpt3-ai-content-generator')?></label>
                ...
                <div class="unique-page-container">
                    <div class="nice-form-group">
                        <label for="composition"><?php echo esc_html__('Composition','gpt3-ai-content-generator')?></label>
                ...
                <div class="unique-page-container">
                    <div class="nice-form-group">
                        <label for="resolution"><?php echo esc_html__('Resolution','gpt3-ai-content-generator')?></label>
                ...
                <div class="unique-page-container">
                    <div class="nice-form-group">
                        <label for="color"><?php echo esc_html__('Color','gpt3-ai-content-generator')?></label>
                ...
                <div class="unique-page-container">
                    <div class="nice-form-group">
                        <label for="special_effects"><?php echo esc_html__('Special Effects','gpt3-ai-content-generator')?></label>
            ...
            <div class="pexel-settings-container" style="display: none;">
                ...
                <div class="unique-page-container">
                    <div class="nice-form-group">
                        <label><?php echo esc_html__('Api Key','gpt3-ai-content-generator')?></label>
                        <input value="<?php echo esc_html($wpaicg_pexels_api)?>" type="text" name="wpaicg_pexels_api" id="wpaicg_pexels_api" class="specific-textfield" onfocus="unmaskValue(this)" onblur="maskValue(this)">
                        <a href="https://www.pexels.com/api/new/" target="_blank"><?php echo esc_html__('Get API Key','gpt3-ai-content-generator')?></a>
                    </div>
                </div>
                <div class="unique-page-container">
                    <div class="nice-form-group">
                        <label><?php echo esc_html__('Orientation','gpt3-ai-content-generator')?></label>
                ...
                <div class="unique-page-container">
                    <div class="nice-form-group">
                        <label><?php echo esc_html__('Size','gpt3-ai-content-generator')?></label>
                ...
                <div class="nice-form-group">
                    <input type="checkbox" id="wpaicg_pexels_enable_prompt" name="wpaicg_pexels_enable_prompt" value="1" <?php checked(1, $wpaicg_pexels_enable_prompt); ?> />
                    <label><?php echo esc_html__('Use Keyword','gpt3-ai-content-generator')?></label>
                   <p>
                    <small><?php echo esc_html__('When enabled, AI picks the main keyword from the title to find relevant images.','gpt3-ai-content-generator')?></small>
                    </p>
                </div>
            ...
            <div class="pixabay-settings-container" style="display: none;">
                ...
                <div class="unique-page-container">
                    <div class="nice-form-group">
                        <label><?php echo esc_html__('Api Key','gpt3-ai-content-generator')?></label>
                        <input value="<?php echo esc_html($wpaicg_pixabay_api)?>" type="text" name="wpaicg_pixabay_api" id="wpaicg_pixabay_api" class="specific-textfield" onfocus="unmaskValue(this)" onblur="maskValue(this)">
                        <a href="https://pixabay.com/api/docs/" target="_blank"><?php echo esc_html__('Get API Key','gpt3-ai-content-generator')?></a>
                    </div>
                </div>
                <div class="unique-page-container">
                    <div class="nice-form-group">
                ...
                <div class="unique-page-container">
                    <div class="nice-form-group">
                        <label><?php echo esc_html__('Type','gpt3-ai-content-generator')?></label>
                ...
                <div class="unique-page-container">
                    <div class="nice-form-group">
                        <label><?php echo esc_html__('Order','gpt3-ai-content-generator')?></label>
            ...
            <!-- Replicate Settings Container -->
            <div class="sd-settings-container" style="display: none;">
                <!-- Replicate Settings Content Here -->
                <div class="unique-page-container">
                    <div class="nice-form-group">
                        <label><?php echo esc_html__('Api Key','gpt3-ai-content-generator')?></label>
                        <input value="<?php echo esc_html($wpaicg_sd_api_key)?>" type="text" name="wpaicg_sd_api_key" id="wpaicg_sd_api_key" class="specific-textfield" onfocus="unmaskValue(this)" onblur="maskValue(this)">
                        <a href="https://replicate.com/account" target="_blank"><?php echo esc_html__('Get API Key','gpt3-ai-content-generator')?></a>
                    </div>
                </div>
                <!-- REPLICATE MODELS -->
                <div class="unique-page-container">
                    <div class="nice-form-group">
                        <label for="wpaicg_replicate_model"><?php echo esc_html__('Model','gpt3-ai-content-generator')?></label>
                        <select id="wpaicg_replicate_model" name="wpaicg_default_replicate_model" class="specific-select">
                            <?php 
                            ...
                            if (!empty($replicate_models)) {
                                foreach ($replicate_models as $owner => $models_group) {
                                    ...
                                    foreach ($models_group as $model) {
                                        // Extract only the model name for comparison
                                        ...
                                        // Output the option with the data-version attribute
                        ...
                        <button id="syncReplicateModelsButton" class="button button-primary wpaicg_sync_replicate" type="button" style="line-height: 3.5;height: 40px">
                ...
                <div class="unique-page-container">
                    <div class="nice-form-group">
                        <label><?php echo esc_html__('Version','gpt3-ai-content-generator')?></label>
                        <input value="<?php echo esc_html($wpaicg_sd_api_version)?>" type="text" name="wpaicg_sd_api_version" class="specific-textfield" placeholder="<?php echo esc_html__('Leave blank for default','gpt3-ai-content-generator')?>">
                    </div>
                </div>
            </div>
        ...
        <section>
            ...
            <p><?php echo esc_html__('AI Assistant is a feature that allows you to add a button to the WordPress editor that will help you to create content. It is compatible with both Gutenberg and Classic Editor.','gpt3-ai-content-generator')?></p>
            ...
            <div class="nice-form-group editor-button-group">
                <?php foreach ($wpaicg_editor_button_menus as $index => $menu): ?>
                    <a href="#" class="editor-settings editor-settings-link" data-index="<?php echo esc_attr($index); ?>">
                        <?php echo esc_html__('Menu', 'gpt3-ai-content-generator') . ' ' . esc_html($index + 1); ?>
                    </a>
                    <div class="assistant-delete-menu-item" data-index="<?php echo esc_attr($index); ?>">x</div>
                    <!-- Hidden inputs to ensure all data is submitted -->
                    <input type="hidden" name="wpaicg_editor_button_menus[<?php echo $index ?>][name]" value="<?php echo esc_attr($menu['name']); ?>">
                    <input type="hidden" name="wpaicg_editor_button_menus[<?php echo $index ?>][prompt]" value="<?php echo esc_attr($menu['prompt']); ?>">
                <?php endforeach; ?>
            </div>
            <div class="nice-form-group newitem">
                <button type="button" class="button button-primary button-large" id="add-assistant-menu-item"><?php echo esc_html__('Add Item','gpt3-ai-content-generator')?></button>
                <button type="button" class="button button-primary button-large" id="toggle-delete-menu-item"><?php echo esc_html__('Delete Item','gpt3-ai-content-generator')?></button>
            </div>
        ...
        <section>
            ...
            <div class="advanced-settings" data-target="comment-settings-container">
                <?php echo esc_html__('Comment Replier', 'gpt3-ai-content-generator')?>
            </div>
            <!-- Search Settings -->
            <div class="advanced-settings" data-target="search-settings-container">
                <?php echo esc_html__('Semantic Search', 'gpt3-ai-content-generator')?>
            </div>
            ...
            <div class="comment-settings-container">
                <div class="unique-page-container">
                    <div class="nice-form-group">
                        <label><?php echo esc_html__('Prompt for Comment Replier ','gpt3-ai-content-generator')?><a href="https://docs.aipower.org/docs/content-writer/comment-replier" target="_blank">?</a></label>
            ...
            <div class="search-settings-container" style="display: none;">
                ...
                <h1><?php echo esc_html__('Search Box Style','gpt3-ai-content-generator')?></h1>
                <div class="unique-page-container">
                    <div class="nice-form-group">
                ...
                <div class="unique-page-container">
                    <div class="nice-form-group">
                        ...
                        <input type="text" name="wpaicg_search_placeholder" value="<?php echo esc_html( get_option( 'wpaicg_search_placeholder', 'Search anything..' ) ); ?>" class="specific-textfield">
                    </div>
                </div>
                <div class="nice-form-group">
                    <label><?php echo esc_html__('Font Color','gpt3-ai-content-generator')?></label>
                    <input value="<?php echo esc_html($wpaicg_search_font_color)?>" type="color" name="wpaicg_search_font_color">
                </div>
                <div class="nice-form-group">
                    <label><?php echo esc_html__('Border Color','gpt3-ai-content-generator')?></label>
                    <input value="<?php echo esc_html($wpaicg_search_border_color)?>" type="color" name="wpaicg_search_border_color">
                </div>
                <div class="nice-form-group">
                    <label><?php echo esc_html__('Background Color','gpt3-ai-content-generator')?></label>
                    <input value="<?php echo esc_html($wpaicg_search_bg_color)?>" type="color" name="wpaicg_search_bg_color">
                </div>
                <div class="unique-page-container">
                    <div class="nice-form-group">
                        <label><?php echo esc_html__('Width','gpt3-ai-content-generator')?></label>
                        <input value="<?php echo esc_html($wpaicg_search_width)?>" min="100" type="text" name="wpaicg_search_width" class="specific-textfield">
                    </div>
                </div>
                <div class="unique-page-container">
                    <div class="nice-form-group">
                        <label><?php echo esc_html__('Height','gpt3-ai-content-generator')?></label>
                        <input value="<?php echo esc_html($wpaicg_search_height)?>" min="100" type="text" name="wpaicg_search_height" class="specific-textfield"> 
                    </div>
                </div>
                <p></p>
                <h1><?php echo esc_html__('Search Results','gpt3-ai-content-generator')?></h1>
                <div class="unique-page-container">
                    <div class="nice-form-group">
                        <label><?php echo esc_html__('Number of Results','gpt3-ai-content-generator')?></label>
                ...
                <div class="nice-form-group">
                    <label><?php echo esc_html__('Font Color','gpt3-ai-content-generator')?></label>
                    <input value="<?php echo esc_html($wpaicg_search_result_font_color)?>" type="color" name="wpaicg_search_result_font_color">
                </div>
                <div class="nice-form-group">
                    <label><?php echo esc_html__('Background Color','gpt3-ai-content-generator')?></label>
                    <input value="<?php echo esc_html($wpaicg_search_result_bg_color)?>" type="color" name="wpaicg_search_result_bg_color">
                </div>
                <div class="nice-form-group">
                    <label><?php echo esc_html__('Progress Color','gpt3-ai-content-generator')?></label>
                    <input value="<?php echo esc_html($wpaicg_search_loading_color)?>" type="color" name="wpaicg_search_loading_color">
                </div>
            </div>
...
<script>
    document.addEventListener("DOMContentLoaded", function () {
        ...
        const seoFrameworkDescription = document.querySelector('#_wpaicg_genesis_description');
        ...
        if (yoastMetaDesc) {
            yoastMetaDesc.addEventListener('click', function () {
                if (this.checked) {
                    ...
                    if (seoFrameworkDescription) seoFrameworkDescription.checked = false;
                }
            });
        }
        ...
        if (seoFrameworkDescription) {
            seoFrameworkDescription.addEventListener('click', function () {
                if (this.checked) {
                    if (aioseoDescription) aioseoDescription.checked = false;
                    if (yoastMetaDesc) yoastMetaDesc.checked = false;
                    if (wpaicgSeoMetaTag) wpaicgSeoMetaTag.checked = false;
...
<script>
    jQuery(document).ready(function($){
        ...
        $('#syncReplicateModelsButton').click(function () {
            var btn = $(this);
            ...
            $.ajax({
                ...
                data: {
                    action: 'wpaicg_fetch_replicate_models',
                ...
                success: function (res){
                    ...
                    if(res.status === 'success'){
                        ...
                        var select = $('#wpaicg_replicate_model');
                        select.empty();
                        
                        $.each(res.models, function(owner, models) {
                            // Add optgroup for the owner
                            var optgroup = $('<optgroup>', { label: owner });
                            ...
                            $.each(models, function(index, model) {
                                var option = $('<option>', {
                                    ...
                                    'data-version': model.latest_version // Attach the version as data attribute
                                });

                                if (model.slug === '<?php echo esc_js($wpaicg_default_replicate_model); ?>') {
        ...
        // Event listener to update the version text field when a model is selected
        document.getElementById('wpaicg_replicate_model').addEventListener('change', function() {
            var selectedOption = this.options[this.selectedIndex];
            var version = selectedOption.getAttribute('data-version');
            
            var apiVersionField = document.querySelector('input[name="wpaicg_sd_api_version"]');
            if (version) {
                apiVersionField.value = version;
            }
        });
