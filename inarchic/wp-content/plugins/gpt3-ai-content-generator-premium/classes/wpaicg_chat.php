...
if(!class_exists('\\WPAICG\\WPAICG_Chat')) {
    class WPAICG_Chat
    {
        ...
        public function __construct()
        {
            ...
            add_action('wp_ajax_wpaicg_block_ip', array($this, 'wpaicg_block_ip'));
            add_action('wp_ajax_wpaicg_empty_log_table', array($this, 'wpaicg_empty_log_table'));
        }

        function wpaicg_empty_log_table() {
            // Check nonce for security
            if (!isset($_POST['_wpnonce']) || !wp_verify_nonce($_POST['_wpnonce'], 'wpaicg_empty_log_table_nonce')) {
                wp_send_json_error(array('message' => esc_html__('Nonce verification failed', 'gpt3-ai-content-generator')));
            }
        
            global $wpdb;
        
            // Run the TRUNCATE command to empty the log table
            $table_name = $wpdb->prefix . 'wpaicg_chatlogs';
            $result = $wpdb->query("TRUNCATE TABLE $table_name");
        
            if ($result === false) {
                wp_send_json_error(array('message' => esc_html__('Failed to empty the log table', 'gpt3-ai-content-generator')));
            } else {
                wp_send_json_success(array('message' => esc_html__('Log table emptied successfully', 'gpt3-ai-content-generator')));
            }
        }

        function wpaicg_block_ip() {
            check_ajax_referer('block_ip_nonce', '_wpnonce'); // Verify nonce
        
            if (isset($_POST['ip'])) {
                $ip = sanitize_text_field($_POST['ip']);
                $banned_ips = get_option('wpaicg_banned_ips', ''); // Get current banned IPs
        
                // Check if the IP is already in the list
                if (strpos($banned_ips, $ip) !== false) {
                    wp_send_json_error(['message' => 'IP is already blocked.']);
                }
        
                // Add the new IP to the list
                if (!empty($banned_ips)) {
                    $banned_ips .= ', ' . $ip; // Append the IP with a comma
                } else {
                    $banned_ips = $ip; // Add IP directly if the list is empty
                }
        
                update_option('wpaicg_banned_ips', $banned_ips); // Save updated IPs to options table
                wp_send_json_success();
            } else {
                wp_send_json_error(['message' => 'No IP provided.']);
            }
        }
        ...
        public function wpaicg_update_chatbot()
        {
            ...
            if(isset($_REQUEST['bot']) && is_array($_REQUEST['bot'])){
                ...
                // Check if feedback is enabled and set save_logs accordingly
                if (isset($bot['feedback_btn']) && ($bot['feedback_btn'] == '1' || $bot['feedback_btn'] === true)) {
                    $bot['save_logs'] = '1';
                }
