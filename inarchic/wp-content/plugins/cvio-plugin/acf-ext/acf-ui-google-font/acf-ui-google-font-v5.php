...
class acf_field_cvio_ui_google_font extends acf_field {
	...
	*/
	...
	function __construct() {
		...
		*/
		...
		$jsonV2 = file_get_contents( plugin_dir_path( __FILE__ ) . 'webfonts-v2.json' );

		$UM_GOOGLEFONTS = json_decode( $json, true );

		$UM_GOOGLEFONTS_V2 = json_decode( $jsonV2, true );
		...
		array_unshift( $UM_GOOGLEFONTS_V2['items'], $none_font );

		$this->font = $UM_GOOGLEFONTS['items'];

		$this->font_v2 = $UM_GOOGLEFONTS_V2['items'];
	...
	*/
	...
	function render_field( $field ) {
		...
		if ( $intValue ) {
			$textValue = $this->font[$intValue]['family'];
		} else {
		...
		<select name="<?php echo esc_attr( $field['name'] ); ?>" class="acf-select-google-fonts">
			<?php foreach ( $this->font_v2 as $key => $font ) : ?>
			<option <?php if ( $textValue == $font['family'] ) : ?>selected="selected"<?php endif; ?> value="<?php echo esc_attr( $font['family'] ); ?>">
				<?php 
				if ( $font["family"] ) :
	...
	*/
	...
	function format_value( $value, $post_id, $field ) {
		...
		foreach ( $this->font_v2 as $key => $font ) {
			if ( $textValue == $font['family'] ) {
				$cur_font = $font;
			}
		}
		if ( $cur_font ) {
