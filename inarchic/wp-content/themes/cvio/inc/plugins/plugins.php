...
function cvio_register_required_plugins() {
	...
	$plugins = array(
		...
		array(
			...
			'slug'		=> 'cvio-plugin',
			'source'	=> CVIO_EXTRA_PLUGINS_DIRECTORY . '/normal/cvio-plugin/230324/' . 'cvio-plugin.zip',
			'required'	=> true,
			'version'	=> '3.3.2',
		),
	...
	if ( class_exists( 'CvioPlugin' ) ) {
		$plugins[] = array(
			'name'					=> esc_html__( 'Advanced Custom Fields Pro', 'cvio' ),
			'slug'					=> 'advanced-custom-fields-pro',
			'source'				=> CVIO_EXTRA_PLUGINS_DIRECTORY . '/normal/acf/260724/' . 'advanced-custom-fields-pro.zip',
			'required'				=> true,
			'version'				=> '6.3.4',
		);
	}
	...
	if ( ! function_exists( 'cvio_plugin_notice' ) ) {
		function cvio_plugin_notice() {
			if ( class_exists( 'CvioPlugin' ) && class_exists( 'ACF' ) ) {
				return;
			} ?>
			<div class="notice notice-info">
				<p><?php echo sprintf( __( 'Please install / activate following plugins: <strong>%s</strong> for get all features of Cvio theme.', 'cvio' ), 'Cvio Plugin, ACF' ); ?></p>
